module XflTests.SymbolItemParseTest
...
[<Fact>]
let SymbolItemParseTest () =
    let SymbolItemString = """
<DOMSymbolItem xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://ns.adobe.com/xfl/2008/" name="body_joint" itemID="65469a09-00000236" sourceLibraryItemHRef="Symbol 1" lastModified="1699125769">
  <timeline>
    <DOMTimeline name="body_joint">
      <layers>
...
"""
    ...
    Document.LoadXml(SymbolItemString)
    
    let SymbolItem = DomSymbolItem(Document.FirstChild)
    
    Assert.Equal("body_joint", SymbolItem.Name)
    Assert.Equal("65469a09-00000236", SymbolItem.ItemId)
    
    Assert.Equal("body_joint", SymbolItem.Timeline.Name)
    Assert.Equal(1, SymbolItem.Timeline.Layers.Count)
    Assert.Equal(1, SymbolItem.Timeline.Layers[0].Frames.Count)
    Assert.Equal(1, SymbolItem.Timeline.Layers[0].Frames[0].Elements.Count)
    
    let element = (SymbolItem.Timeline.Layers[0].Frames[0]).Elements[0]
    Assert.Equal(IDomFrameElement.ElementType.Shape, element.Type)
    
    let shape = element :?> DomShape
    ...
    // we should only get 1 edge out of that
