module XflTests.DocumentParseTest
...
"""
    ...
    let FlashDoc = DomDocument(Document.FirstChild)
    ...
    let mutable i = 0
    for symItem in FlashDoc.Symbols do
         Assert.Equal(expectedSymbols[i], symItem.HRef)
         i <- i + 1
    
    // there is 1 group in this document for each
    Assert.Equal(1, FlashDoc.SwatchGroups.Count)
    Assert.Equal(1, FlashDoc.SwatchGroupsExtended.Count)
    
    // swatch items
    // 252 items in total
    // 26 should be blank/black
    Assert.Equal(252, FlashDoc.SwatchGroups[0].Swatches.Count)
    
    i <- 0
    for swItem in FlashDoc.SwatchGroups[0].Swatches do
    ...
    // Extended items
    // 7 in total
    Assert.Equal(7, FlashDoc.SwatchGroupsExtended[0].Swatches.Count)